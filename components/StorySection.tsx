...
const CabinIcon = () => (
  <svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round">
...
const StarFlagIcon = () => (
  <svg viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round">
    ...
    <path d="M14 10l1.5-3 1.5 3 3 .5-2 2 .5 3-3-1.5-3 1.5.5-3-2-2 3-.5z" strokeWidth="1" fill="currentColor" opacity="0.3" />
  </svg>
);
    ...
    >
      <div className={styles["story-container"]}>
        ...
        <header className={styles["story-header"]}>
          ...
          <h2 id="story-heading" className={styles["story-headline"]}>The Road to Tennessee</h2>

          {/* Animated year counter */}
          <div className={`${styles["story-counter"]} ${isVisible ? styles["story-counter--visible"] : ''}`}>
            <span className={styles["story-counter-number"]}>{yearCount}</span>
            <span className={styles["story-counter-unit"]}>Years</span>
          </div>
          <p className={styles["story-counter-caption"]}>From frontier settlement to American statehood</p>
        ...
        <div className={styles["story-trail"]} aria-hidden="true">
          <div className={styles["story-trail-path"]}>
            <svg viewBox="0 0 400 60" preserveAspectRatio="none">
              ...
              <circle cx="308" cy="32" r="6" fill="currentColor" className={styles["story-trail-marker"]} />
              <circle cx="400" cy="25" r="4" fill="currentColor" opacity="0.4" />
            </svg>
          </div>
          <div className={styles["story-trail-labels"]}>
            <span className={`${styles["story-trail-label"]} ${styles["story-trail-label--start"]}`}>Philadelphia<br /><small>1776</small></span>
            <span className={`${styles["story-trail-label"]} ${styles["story-trail-label--mid"]}`}>Rocky Mount<br /><small>1790</small></span>
            <span className={`${styles["story-trail-label"]} ${styles["story-trail-label--end"]}`}>Statehood<br /><small>1796</small></span>
          </div>
        </div>
        ...
        <div className={styles["story-timeline-wrapper"]}>
          ...
          <div className={styles["story-timeline-line"]} aria-hidden="true">
            <span className={`${styles["story-timeline-line-fill"]} ${isVisible ? styles["story-timeline-line-fill--visible"] : ''}`} />
          </div>

          <ol className={styles["story-timeline"]} role="list" aria-label="Rocky Mount timeline">
            {milestones.map((milestone, index) => (
              ...
              >
                {/* Timeline node */}
                <span className={`${styles["story-milestone-node"]} ${milestone.isHero ? styles["story-milestone-node--hero"] : ''}`} aria-hidden="true" />
        ...
        {/* Footer */}
        <footer className={`${styles["story-footer"]} ${isVisible ? styles["story-footer--visible"] : ''}`}>
