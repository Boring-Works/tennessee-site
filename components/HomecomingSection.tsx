...
const HorizonSilhouette = () => (
  <svg viewBox="0 0 1200 200" preserveAspectRatio="none" fill="currentColor">
    {/* Rolling hills */}
    <path d="M0 200 L0 140 Q100 100 200 120 T400 100 T600 130 T800 90 T1000 110 T1200 80 L1200 200 Z" opacity="0.15" />
    <path d="M0 200 L0 160 Q150 130 300 150 T600 120 T900 140 T1200 100 L1200 200 Z" opacity="0.25" />
    ...
    <g transform="translate(580, 95)">
      ...
      <path d="M-5 45 L20 20 L45 45" fill="none" stroke="currentColor" strokeWidth="3" /> {/* Roof */}
...
export default function HomecomingSection() {
  ...
  return (
    ...
    >
      ...
      <div className={`${styles["homecoming-horizon"]} ${isVisible ? styles["homecoming-horizon--visible"] : ''}`} aria-hidden="true">
        <HorizonSilhouette />
      </div>

      {/* Decorative border frame */}
      <div className={styles["homecoming-frame"]} aria-hidden="true">
        <span className={`${styles["homecoming-frame-corner"]} ${styles["homecoming-frame-corner--tl"]}`} />
        <span className={`${styles["homecoming-frame-corner"]} ${styles["homecoming-frame-corner--tr"]}`} />
        <span className={`${styles["homecoming-frame-corner"]} ${styles["homecoming-frame-corner--bl"]}`} />
        <span className={`${styles["homecoming-frame-corner"]} ${styles["homecoming-frame-corner--br"]}`} />
      </div>

      <div className={styles["homecoming-container"]}>
        {/* Location eyebrow */}
        <p className={`${styles["homecoming-eyebrow"]} ${isVisible ? styles["homecoming-eyebrow--visible"] : ''}`}>
          <svg className={styles["homecoming-pin"]} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
        ...
        {/* Experiential hook */}
        <p className={`${styles["homecoming-hook"]} ${isVisible ? styles["homecoming-hook--visible"] : ''}`}>
        ...
        <div className={`${styles["homecoming-map-card"]} ${isVisible ? styles["homecoming-map-card--visible"] : ''}`}>
          {/* Torn edge top */}
          <div className={`${styles["homecoming-map-tear"]} ${styles["homecoming-map-tear--top"]}`} aria-hidden="true" />
          ...
          {/* Torn edge bottom */}
          <div className={`${styles["homecoming-map-tear"]} ${styles["homecoming-map-tear--bottom"]}`} aria-hidden="true" />
        </div>

        {/* Period signpost info bar */}
        <div className={`${styles["homecoming-signpost"]} ${isVisible ? styles["homecoming-signpost--visible"] : ''}`}>
          <div className={`${styles["homecoming-signpost-plank"]} ${styles["homecoming-signpost-plank--hours"]}`}>
            <span className={styles["homecoming-signpost-label"]}>Hours</span>
            <span className={styles["homecoming-signpost-value"]}>Tue–Sat 10am–5pm · Sun 1pm–5pm</span>
          </div>
          <div className={styles["homecoming-signpost-post"]} aria-hidden="true" />
          <div className={`${styles["homecoming-signpost-plank"]} ${styles["homecoming-signpost-plank--admission"]}`}>
            <span className={styles["homecoming-signpost-label"]}>Admission</span>
            <span className={styles["homecoming-signpost-value"]}>Adults $8 · Children $5 · Under 6 free</span>
          </div>
        </div>

        {/* Pulsing CTA with wax seal */}
        <div className={`${styles["homecoming-cta-wrapper"]} ${isVisible ? styles["homecoming-cta-wrapper--visible"] : ''}`}>
          <Link href="/visit" className={styles["homecoming-cta"]}>
            <span className={styles["homecoming-cta-seal"]} aria-hidden="true">★</span>
            <span className={styles["homecoming-cta-text"]}>Visit Rocky Mount</span>
          </Link>
        </div>

        {/* Animated closing tagline */}
        <p className={`${styles["homecoming-closing"]} ${showTagline ? styles["homecoming-closing--visible"] : ''}`}>
